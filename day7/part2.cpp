...
int main() {
    ...
    if (datafile.is_open()) {
        while(datafile.good() ) {
            ...
            if(jokers != 0) {
                ...
                if(jokers == 5) {
                   five_of_a_kind.push_back({0101010101, bid});
                   continue;
                }
                else {
                    int biggest = -1;
                    ...
                    for(int i = 0; i < card_values.size(); i++) {
                        if(card_values[i][1] > biggestamount) {
                            biggest = i;
                            biggestamount = card_values[i][1];
                        }
                    }
                    card_values[biggest][1] += jokers;
                }
    ...
    for(int i = 0; i < high_card.size(); i++) {
        std::cout << "rank " << rank << ": " << high_card[i][0] << " " << high_card[i][1]  << " (high card)" << std::endl;
        result += rank * high_card[i][1];
        rank++;
    }
    for(int i = 0; i < one_pair.size(); i++) {
        std::cout << "rank " << rank << ": " << one_pair[i][0] << " " << one_pair[i][1]  << " (one pair)" << std::endl;
        result += rank * one_pair[i][1];
        rank++;
    }
    for(int i = 0; i < two_pair.size(); i++) {
        std::cout << "rank " << rank << ": " << two_pair[i][0] << " " << two_pair[i][1] << " (two pair)" << std::endl;
        result += rank * two_pair[i][1];
        rank++;
    }
    for(int i = 0; i < three_of_a_kind.size(); i++) {
        std::cout << "rank " << rank << ": " << three_of_a_kind[i][0] << " " << three_of_a_kind[i][1] << " (3 of a kind)" << std::endl;
        result += rank * three_of_a_kind[i][1];
        rank++;
    }
    for(int i = 0; i < full_house.size(); i++) {
        std::cout << "rank " << rank << ": " << full_house[i][0] << " " << full_house[i][1] << " (full house)" << std::endl;
        result += rank * full_house[i][1];
        rank++;
    }
    for(int i = 0; i < four_of_a_kind.size(); i++) {
        std::cout << "rank " << rank << ": " << four_of_a_kind[i][0] << " " << four_of_a_kind[i][1] << " (4 of a kind)" << std::endl;
        result += rank * four_of_a_kind[i][1];
        rank++;
    }
    for(int i = 0; i < five_of_a_kind.size(); i++) {
        std::cout << "rank " << rank << ": " << five_of_a_kind[i][0] << " " << five_of_a_kind[i][1] << " (5 of a kind)" << std::endl;
