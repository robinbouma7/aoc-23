    ...
    if (datafile.is_open()) {
        while(datafile.good() ) {
            ...
            for(int i = grid.size() - 1; i > 0; i--) {
                int num1 = grid[i][grid[i].size() - 1];
                int num2 = grid[i - 1][grid[i - 1].size() - 1];
                grid[i - 1].push_back(num1 + num2);
            }
            std::cout << "final number: " << grid[0][grid[0].size() - 1] << std::endl;	 
            nextnums.push_back(grid[0][grid[0].size() - 1]); 
            result += grid[0][grid[0].size() - 1];
